...
public class Ont_Configure_Wan_Class implements Serializable {
    private static final Pattern INTERFACE_PATTERN = Pattern.compile("it_nr\\(\"(.*?)\",", 40);
    ...
    private JSONObject getVoipParams(String str) {
        ...
        try {
            ...
            Element first3 = document.getElementsByAttributeValue("name", "proxy1_domain_name").first();
            String str5 = "";
            if (first == null) {
                str2 = str5;
            } else {
    ...
    public JSONObject extractWanParameter(String str) {
        try {
            Element selectFirst = Jsoup.connect(str + "/net_eth_links.asp").get().selectFirst("script:not([src])");
            if (selectFirst == null) {
