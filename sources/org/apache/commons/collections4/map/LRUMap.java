...
public class LRUMap<K, V> extends AbstractLinkedMap<K, V> implements BoundedMap<K, V>, Serializable, Cloneable {
    ...
    public void addMapping(int i, int i2, K k, V v) {
        ...
        if (isFull()) {
            ...
            if (this.scanUntilRemovable) {
                ...
                if (linkEntry == null) {
                    throw new IllegalStateException("Entry.after=null, header.after=" + this.header.after + " header.before=" + this.header.before + " key=" + k + " value=" + v2 + " size=" + this.size + " maxSize=" + this.maxSize + " This should not occur if your keys are immutable, and you have used synchronization properly.");
                }
            } else {
            ...
            if (!z) {
                ...
                throw new IllegalStateException("reuse=null, header.after=" + this.header.after + " header.before=" + this.header.before + " key=" + k + " value=" + v2 + " size=" + this.size + " maxSize=" + this.maxSize + " This should not occur if your keys are immutable, and you have used synchronization properly.");
            }
        } else {
            super.addMapping(i, i2, k, v);
        }
    ...
    public void reuseMapping(AbstractLinkedMap.LinkEntry<K, V> linkEntry, int i, int i2, K k, V v) {
        ...
        try {
            ...
            StringBuilder append = new StringBuilder("NPE, entry=").append(linkEntry).append(" entryIsHeader=");
            if (linkEntry != this.header) {
                z = false;
            }
            throw new IllegalStateException(append.append(z).append(" key=").append(k).append(" value=").append(v).append(" size=").append(this.size).append(" maxSize=").append(this.maxSize).append(" This should not occur if your keys are immutable, and you have used synchronization properly.").toString());
        }
    }
