...
public class ChangeSetPerformer {
    ...
    private ChangeSetResults perform(ArchiveEntryIterator archiveEntryIterator, ArchiveOutputStream archiveOutputStream) throws IOException {
        ...
        while (archiveEntryIterator.hasNext()) {
            ...
            while (true) {
                ...
                if (type != 1 || name == null) {
                    if (type == 4 && name != null && name.startsWith(change2.targetFile() + PackagingURIHelper.FORWARD_SLASH_STRING)) {
    ...
    private boolean isDeletedLater(Set<Change> set, ArchiveEntry archiveEntry) {
        ...
        for (Change next : set) {
            ...
            if (type == 4 && name.startsWith(targetFile + PackagingURIHelper.FORWARD_SLASH_STRING)) {
                return true;
            }
        }
        return false;
