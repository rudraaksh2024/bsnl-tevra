...
public abstract class JavaHexBinaryHolderEx extends JavaHexBinaryHolder {
    ...
    public static void validateValue(byte[] bArr, SchemaType schemaType, ValidationContext validationContext) {
        ...
        if (!(facet == null || (intValue3 = ((XmlObjectBase) facet).getBigIntegerValue().intValue()) == bArr.length)) {
            validationContext.invalid(XmlErrorCodes.DATATYPE_LENGTH_VALID$BINARY, new Object[]{XmlErrorCodes.HEXBINARY, Integer.valueOf(bArr.length), Integer.valueOf(intValue3), QNameHelper.readable(schemaType)});
        }
        ...
        if (facet2 != null && (intValue2 = ((XmlObjectBase) facet2).getBigIntegerValue().intValue()) > bArr.length) {
            validationContext.invalid(XmlErrorCodes.DATATYPE_MIN_LENGTH_VALID$BINARY, new Object[]{XmlErrorCodes.HEXBINARY, Integer.valueOf(bArr.length), Integer.valueOf(intValue2), QNameHelper.readable(schemaType)});
        }
        ...
        if (facet3 != null && (intValue = ((XmlObjectBase) facet3).getBigIntegerValue().intValue()) < bArr.length) {
            validationContext.invalid(XmlErrorCodes.DATATYPE_MAX_LENGTH_VALID$BINARY, new Object[]{XmlErrorCodes.HEXBINARY, Integer.valueOf(bArr.length), Integer.valueOf(intValue), QNameHelper.readable(schemaType)});
        }
