...
public class Jsr173 {
    ...
    public static XMLStreamReader newXmlStreamReader(Cur cur, XmlOptions xmlOptions) {
        XMLStreamReaderBase xMLStreamReaderBase;
        XmlOptions maskNull = XmlOptions.maskNull(xmlOptions);
        boolean z = maskNull.isSaveInner() && !maskNull.isSaveOuter();
        int kind = cur.kind();
        if (kind == 0 || kind < 0) {
            xMLStreamReaderBase = new XMLStreamReaderForString(cur, cur.getChars(-1), cur._offSrc, cur._cchSrc);
        } else if (!z) {
            xMLStreamReaderBase = new XMLStreamReaderForNode(cur, false);
        } else if (cur.hasAttrs() || cur.hasChildren()) {
            xMLStreamReaderBase = new XMLStreamReaderForNode(cur, true);
        } else {
            xMLStreamReaderBase = new XMLStreamReaderForString(cur, cur.getFirstChars(), cur._offSrc, cur._cchSrc);
        }
        if (cur._locale.noSync()) {
            return new UnsyncedJsr173(cur._locale, xMLStreamReaderBase);
        }
        return new SyncedJsr173(cur._locale, xMLStreamReaderBase);
    }
    ...
    private static abstract class XMLStreamReaderBase implements XMLStreamReader, NamespaceContext, Location {
        ...
        public void require(int i, String str, String str2) throws XMLStreamException {
            ...
            if (i != getEventType()) {
                throw new XMLStreamException();
            } else if (str != null && !getNamespaceURI().equals(str)) {
                throw new XMLStreamException();
            } else if (str2 != null && !getLocalName().equals(str2)) {
                throw new XMLStreamException();
            }
        }
        ...
        public String getPrefix(String str) {
            ...
            String prefixForNamespace = streamCur.prefixForNamespace(str, (String) null, false);
            streamCur.pop();
