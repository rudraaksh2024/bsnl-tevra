...
final class LinkTable {
    ...
    public LinkTable(int i, WorkbookRecordList workbookRecordList) {
        ...
        int findFirstRecordLocBySid = findFirstRecordLocBySid(140);
        if (findFirstRecordLocBySid >= 0 || findFirstRecordLocBySid(SSTRecord.sid) - 1 >= 0) {
    ...
    public void addName(NameRecord nameRecord) {
        ...
        if (findFirstRecordLocBySid == -1) {
            findFirstRecordLocBySid = findFirstRecordLocBySid(140);
        }
    ...
    public String[] getExternalBookAndSheetName(int i) {
        ...
        String str2 = firstSheetIndexFromRefIndex >= 0 ? externalBookRecord.getSheetNames()[firstSheetIndexFromRefIndex] : null;
        if (lastSheetIndexFromRefIndex >= 0) {
            str = externalBookRecord.getSheetNames()[lastSheetIndexFromRefIndex];
        }
        if (firstSheetIndexFromRefIndex == lastSheetIndexFromRefIndex) {
            return new String[]{externalBookRecord.getURL(), str2};
        }
        return new String[]{externalBookRecord.getURL(), str2, str};
    }
    ...
    public int checkExternSheet(int i, int i2) {
        ...
        while (true) {
            ...
            if (i3 >= externalBookBlockArr.length) {
                i3 = -1;
                break;
            } else if (externalBookBlockArr[i3].getExternalBookRecord().isInternalReferences()) {
                break;
            } else {
                i3++;
            }
        ...
        if (i3 >= 0) {
            ...
            if (externSheetRecord != null) {
                int refIxForSheet = externSheetRecord.getRefIxForSheet(i3, i, i2);
                if (refIxForSheet >= 0) {
                    return refIxForSheet;
                }
                return this._externSheetRecord.addRef(i3, i, i2);
            }
    ...
    private int findFirstRecordLocBySid(short s) {
        int i = 0;
        for (Record sid : this._workbookRecordList.getRecords()) {
            if (sid.getSid() == s) {
                return i;
            }
            i++;
    ...
    public String resolveNameXText(int i, int i2, InternalWorkbook internalWorkbook) {
        ...
        } else if (this._externalBookBlocks[extbookIndexFromRefIndex]._externalNameRecords.length > i2) {
    ...
    public NameXPtg addNameXPtg(String str) {
        ...
        for (Record next : this._workbookRecordList.getRecords()) {
