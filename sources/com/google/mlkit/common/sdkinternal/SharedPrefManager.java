...
public class SharedPrefManager {
    public static final Component<?> COMPONENT = Component.builder(SharedPrefManager.class).add(Dependency.required((Class<?>) MlKitContext.class)).add(Dependency.required((Class<?>) Context.class)).factory(zzs.zza).build();
    ...
    public synchronized void clearIncompatibleModelInfo(RemoteModel remoteModel) {
        zza().edit().remove(String.format("bad_hash_%s", new Object[]{remoteModel.getUniqueModelNameForPersist()})).remove("app_version").apply();
    }
    ...
    public synchronized void setDownloadingModelInfo(long j, ModelInfo modelInfo) {
        ...
        zza().edit().putString(String.format("downloading_model_hash_%s", new Object[]{modelNameForPersist}), modelHash).putLong(String.format("downloading_model_id_%s", new Object[]{modelNameForPersist}), j).putLong(String.format("downloading_begin_time_%s", new Object[]{modelNameForPersist}), SystemClock.elapsedRealtime()).apply();
    }

    public synchronized void setIncompatibleModelInfo(RemoteModel remoteModel, String str, String str2) {
        zza().edit().putString(String.format("bad_hash_%s", new Object[]{remoteModel.getUniqueModelNameForPersist()}), str).putString("app_version", str2).apply();
    }
